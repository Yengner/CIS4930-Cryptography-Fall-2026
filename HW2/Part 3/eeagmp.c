...
void extended_euclid(mpz_t inverse, mpz_t number, mpz_t modulo)
{
    mpz_t oldR, r, oldT, t, q, remainder, nextT, temp, negativeOne;
    mpz_inits(oldR, r, oldT, t, q, remainder, nextT, temp, negativeOne, NULL);
    mpz_mod(oldR, number, modulo);
    mpz_set(r, modulo);
    mpz_set_ui(oldT, 1);
    mpz_set_ui(t, 0);
    mpz_set_str(negativeOne, "-1", 10);

    while (mpz_sgn(r) != 0)
    {
        mpz_fdiv_q(q, oldR, r);
        mpz_mod(remainder, oldR, r);
        mpz_mul(temp, q, t);
        mpz_mul(temp, temp, negativeOne);
        mpz_add(nextT, oldT, temp);
    ...
    mpz_add(temp, oldR, negativeOne);
    if (mpz_sgn(temp) == 0)
        mpz_mod(inverse, oldT, modulo);
    else
        mpz_set_str(inverse, "-1", 10);

    mpz_clears(oldR, r, oldT, t, q, remainder, nextT, temp, negativeOne, NULL);
}
...
int main(int argc, char *argv[])
{
    if (argc != 3)
    {
        printf("Usage: ./eeagmp numberfile modulofile\n");
        return 1;
    }

    int numberLen, moduloLen;
    unsigned char *numberText = Read_File(argv[1], &numberLen);
    unsigned char *moduloText = Read_File(argv[2], &moduloLen);

    mpz_t number, modulo, inverse;
    mpz_inits(number, modulo, inverse, NULL);
    if (mpz_set_str(number, (char *)numberText, 10) != 0 ||
        mpz_set_str(modulo, (char *)moduloText, 10) != 0 ||
        mpz_sgn(modulo) <= 0)
    {
    ...
    extended_euclid(inverse, number, modulo);
    char resultText[100];
    mpz_get_str(resultText, 10, inverse);
    Write_File("Result.txt", resultText);

    mpz_clears(number, modulo, inverse, NULL);
    free(numberText);
    free(moduloText);
    return 0;
}
